...
for i in range(len(base_dataset)):
    ...
    predicted_action = base_policy.predict_action_chunk(
        {
            "observation.images.wrist": base_dataset[i]["observation.images.wrist"].unsqueeze(0).to("cuda"),
            "observation.images.top": base_dataset[i]["observation.images.top"].unsqueeze(0).to("cuda"),
